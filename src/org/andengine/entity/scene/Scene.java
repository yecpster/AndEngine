...
public class Scene extends Entity {
    ...
    /**
     * Enable or disable the binding of TouchAreas to PointerIDs (fingers). When enabled: TouchAreas get bound to a PointerID (finger) when returning true in
     * {@link IShape#onAreaTouched(TouchEvent, float, float)} or {@link IOnAreaTouchListener#onAreaTouched(TouchEvent, ITouchArea, float, float)} with
     * {@link TouchEvent#ACTION_DOWN}, they will receive all subsequent {@link TouchEvent}s that are made with the same PointerID (finger) <b>even if the
     * {@link TouchEvent} is outside of the actual {@link ITouchArea}</b>!
     * 
     ...
     * Enable or disable the binding of TouchAreas to PointerIDs (fingers). When enabled: TouchAreas get bound to a PointerID (finger) when returning true in
     * {@link IShape#onAreaTouched(TouchEvent, float, float)} or {@link IOnAreaTouchListener#onAreaTouched(TouchEvent, ITouchArea, float, float)} with
     * {@link TouchEvent#ACTION_MOVE}, they will receive all subsequent {@link TouchEvent}s that are made with the same PointerID (finger) <b>even if the
     * {@link TouchEvent} is outside of the actual {@link ITouchArea}</b>!
     * 
    ...
    /**
     * Enable or disable the binding of TouchAreas to PointerIDs (fingers). When enabled: The OnSceneTouchListener gets bound to a PointerID (finger) when
     * returning true in {@link Shape#onAreaTouched(TouchEvent, float, float)} or
     * {@link IOnAreaTouchListener#onAreaTouched(TouchEvent, ITouchArea, float, float)} with {@link TouchEvent#ACTION_DOWN}, it will receive all subsequent
     * {@link TouchEvent}s that are made with the same PointerID (finger) <b>even if the {@link TouchEvent} is would belong to an overlaying {@link ITouchArea}
     * </b>!
     * 
    ...
    public boolean onSceneTouchEvent(final TouchEvent pSceneTouchEvent) {
        ...
        if (!isActionDown) {
            ...
            if (this.mTouchAreaBindingOnActionDownEnabled) {
                ...
                /*
                 * In the case a ITouchArea has been bound to this PointerID, we'll pass this this TouchEvent to the same ITouchArea.
                 */
                ...
                if (this.mOnAreaTouchTraversalBackToFront) { /* Back to Front. */
                    for (int i = 0; i < touchAreaCount; i++) {
                        ...
                        if (touchArea.contains(sceneTouchEventX, sceneTouchEventY)) {
                            ...
                            if (handled != null && handled) {
                                /*
                                 * If binding of ITouchAreas is enabled and this is an ACTION_DOWN event, bind this ITouchArea to the PointerID.
                                 */
        ...
        if (this.mOnSceneTouchListener != null) {
            ...
            if (handled != null && handled) {
                /*
                 * If binding of ITouchAreas is enabled and this is an ACTION_DOWN event, bind the active OnSceneTouchListener to the PointerID.
                 */
