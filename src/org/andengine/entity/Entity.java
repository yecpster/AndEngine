...
public class Entity implements IEntity {
    ...
    /**
     * @param pAlpha
     *            from <code>0.0f</code> (transparent) to <code>1.0f</code> (opaque)
     */
    @Override
    ...
    public Transformation getLocalToParentTransformation() {
        ...
        if (this.mLocalToParentTransformationDirty) {
            ...
            if ((scaleX != 1) || (scaleY != 1)) {
                ...
                /*
                 * TODO Check if it is worth to check for scaleCenterX == 0 && scaleCenterY == 0 as the two postTranslate can be saved. The same obviously
                 * applies for all similar occurrences of this pattern in this class.
                 */
    ...
    /**
     * @param pGLState
     *            the currently active {@link GLState} i.e. to apply transformations to.
     * @param pCamera
     *            the currently active {@link Camera} i.e. to be used for culling.
     */
    ...
    protected void applyRotation(final GLState pGLState) {
        ...
        if (rotation != 0) {
            ...
            /*
             * TODO There is a special, but very likely case when mRotationCenter and mScaleCenter are the same. In that case the last glTranslatef of the
             * rotation and the first glTranslatef of the scale is superfluous. The problem is that applyRotation and applyScale would need to be "merged" in
             * order to efficiently check for that condition.
             */
        }
    }
    ...
    private void assertEntityHasNoParent(final IEntity pEntity) throws IllegalStateException {
        if (pEntity.hasParent()) {
            ...
            throw new IllegalStateException("pEntity '" + entityClassName + "' already has a parent: '" + currentParentClassName + "'. New parent: '"
                    + newParentClassName + "'!");
        }
    }
